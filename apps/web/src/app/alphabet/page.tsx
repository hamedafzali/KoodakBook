...
export default async function AlphabetPage() {
  ...
  const items: ExplorerItem[] = letters.map((l) => ({
    ...
    detailTitle: l.example_word
      ? `${l.character} برای ${l.example_word.persian}`
      : l.name_persian,
  ...
  const jsonLd = {
    ...
    description:
      "تمام ۳۲ حرف الفبای فارسی، هر کدام قابل شنیدن با صدای واقعی — رایگان و بدون ثبت‌نام.",
    inLanguage: "fa",
    ...
    hasPart: {
      ...
      itemListElement: letters.map((l, i) => ({
        ...
        ...(l.audio_url
          ? { audio: { "@type": "AudioObject", contentUrl: l.audio_url } }
          : {}),
      })),
    },
  };
    ...
    <div className="min-h-screen bg-warm-white">
      ...
      <header className="bg-brand-gradient-br text-white">
        <div className="max-w-2xl mx-auto px-5 pt-10 pb-8 text-center">
          ...
          <p className="mt-2 text-white/90 text-sm sm:text-base">
            ۳۲ حرف · بزن و بشنو · رایگان و بدون ثبت‌نام
          </p>
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-5 -mt-5 pb-16">
        <div className="bg-white rounded-3xl shadow-lg border border-slate-100 p-4 sm:p-6">
          {letters.length === 0 ? (
            <p className="text-center text-slate-500 py-10">
              در حال حاضر نمی‌توان حروف را بارگذاری کرد.
            </p>
          ) : (
            <TapToHearExplorer
              items={items}
        ...
        <p className="mt-8 text-center text-sm text-slate-500 max-w-md mx-auto">
          کودک‌بوک آموزش فارسی را با صداکشی (ترکیب صداها) شروع می‌کند — همان
          روشی که علم خواندن آن را تأیید کرده.{" "}
          <a href="/" className="text-brand-text font-bold hover:underline">
            درباره کودک‌بوک بیشتر بدانید
          </a>
        </p>
      </main>
    </div>
