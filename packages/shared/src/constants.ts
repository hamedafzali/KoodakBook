...
/* @deprecated — no longer used by any screen (2026-09 design pass).
 *
 * These were dealt out by list index, so a card's colour changed whenever the
 * list reordered: five accents that looked like signal and carried none. Colour
 * now means the MODULE, and every surface takes its hue from the 4-stop ramps
 * in apps/web/src/app/globals.css (generated by tools/design/ramps.py), where
 * the white-text pairing is solved rather than checked afterwards.
 *
 * Kept only so an out-of-tree consumer doesn't break on upgrade. Do not use it
 * in new code; reach for a ramp. */
